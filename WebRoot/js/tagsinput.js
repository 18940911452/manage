 ...
 */
  ...
  TagsInput.prototype = {
    ...
    add: function(item, dontPushVal, options) {
      ...
      $tag.data('item', item);
      if($('.bootstrap-tagsinput .badge-info').length>=1){
      	swal({ 
		   title: "主题只能添加一个",
		   type: "error",
		   timer: 2000,
      ...
      var num=0;
      for(var i=0;i<arr;i++){
      	if($('.tagsinputBox li').eq(i).html()==item){
      		num++
      	}
      }
      if(num==0){
      	swal({ 
		   title: "没有配置该主题，点击确定可直接配置", 
		   type: "warning",
		  ...
		  if (isConfirm) {
		  	$(this).parent().remove()
			$.post('report/mod2/addTopicalInfo',{top_name:item},function(data){
				console.log(data)
