...
//  this.undockInitiatorContent = new dockspawn.UndockInitiator(this.contentElement, dockspawn.TabHandle.prototype._performUndock.bind(this););
//  this.undockInitiatorContent.enabled = false;
//  undockInitiatorArr.push(this.undockInitiatorContent)
...
//      event.clientX - (dialogWidth-10),
        event.clientY - (dragOffset.y-150));
...
//zyw
	if (height == 0) {
		var height_ = getHeight[width]
		console.log(getHeight)
		console.log(this.containerType)
		//debugger
		this.splitterPanel.resize(width,650);
	    this._cachedWidth = width;
	    this._cachedHeight = 650;
	    return;
	}
...
Object.defineProperty(dockspawn.SplitterDockContainer.prototype, "height", {
    get: function()
    {
    	//zyw  2017-11-21 15:32:13 修复 ‘horizontal’高度获取问题
//      if (this._cachedHeight === undefined)
//         console.log(this);console.log(this._cachedHeight);console.log(this.splitterPanel.panelElement.firstChild.clientHeight);this._cachedHeight = this.splitterPanel.panelElement.clientHeight;
        if (this._cachedHeight === undefined)
           this._cachedHeight = this.splitterPanel.panelElement.firstChild.clientHeight
...
{
	//zyw
	//2017-10-29 12:17:34 添加
//	state.elementContent = this.elementContent;
//	state._cachedHeight = this._cachedHeight;
//	state._cachedWidth = this._cachedWidth;
    state.element = this.elementContent.id;
    state.width = this.width;
    state.height = this.height;
    state.id = $(this.elementContent).data('id');
    state.sid = $(this.elementContent).data('sid');
    state.autoid = $(this.elementContent).data('autoid');
    state.name = $(this.elementContent).attr('caption');
    state.style = $(this.elementContent).children('div').attr('class');
    state.border = $(this.elementContent).data('border');
    if ($(this.elementContent).attr('content') != '') {
    	state.content = $(this.elementContent).attr('content');
    }
};
...
{
	//zyw
	getHeight[state.width] = state.height
    this.width = state.width;
    this.height = state.height;
    this.resize(this.width, this.height);
};
...
//  this.elementButtonClose.innerHTML = '<i class="icon-remove"></i>';
    ...
    this.elementContentHost.classList.add("dock-content");
    ...
    this.undockInitiator.enabled = true;
    this.undockInitiatorContent = new dockspawn.UndockInitiator(this.elementContentHost, this.performUndockToDialog.bind(this));
    this.undockInitiatorContent.enabled = false;
    undockInitiatorArr.push(this.undockInitiatorContent)
...
//  }
    //zyw
//  if (height<1) {
//  	this._setPanelDimensions(width, this._cachedHeight);
//  }else{
...
{
    //2018-06-08 18:30:38  zyw 大屏title拖动问题，调整title的高度，使content也能支持拖动
    this.elementTitle.style.height = height + "px";
...
//  this.elementContentHost.style.height = contentHeight + "px";
    this.elementContentHost.style.height = height + "px";
    this.elementContent.style.height = height + "px";
...
{
	//zyw 2017-11-28 11:12:07 修改 删除bug
	this.performUndock();
    this.destroy();
    destroyViewBoxDropdown(this);
...
dockspawn.DockGraphDeserializer.prototype.deserialize = function(json)
{
    ...
    console.log(model.rootNode)
    childInfoEach(model.rootNode)
    return model;
};
